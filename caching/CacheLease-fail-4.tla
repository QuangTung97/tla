...
Start:
    either
        ...
        new_value := db_value;
    SetCache:
        if (cache_value = 0 /\ cache_lease /= 0) \/ (cache_value /= 0 /\ new_value > cache_value) then
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "2f845c4a" /\ chksum(tla) = "fbf4dfd2")
...
SetCache(self) == /\ pc[self] = "SetCache"
                  /\ IF (cache_value = 0 /\ cache_lease /= 0) \/ (cache_value /= 0 /\ new_value[self] > cache_value)
...
\* Last modified Mon Feb 22 09:19:30 ICT 2021 by teko
